...
pub const KATA_ANNO_CFG_HYPERVISOR_ENABLE_HUGE_PAGES: &str =
        ...
        for (key, value) in &self.annotations {
            if hv.security_info.is_annotation_enabled(key) {
                match key.as_str() {
                    ...
                    KATA_ANNO_CFG_HYPERVISOR_ENABLE_HUGE_PAGES => match self.get_value::<bool>(key)
                    {
                        Ok(r) => {
                    ...
                    KATA_ANNO_CFG_HYPERVISOR_HUGEPAGE_TYPE => {
                        match self.get_value::<HugePageType>(key) {
                            ...
                            Err(_e) => {
                                return Err(io::Error::new(
                                    io::ErrorKind::InvalidData,
                                    format!("parse huge pages error: {}", value),
                                ));
                            }
                        }
                    }
